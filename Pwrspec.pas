...
type
  ...
  TPwrSpecFrm = class(TForm)
    ...
    prVarProgress: TProgressBar;
    prSpecProgress: TProgressBar;
    ...
    procedure plVarPlotCursorChange(Sender: TObject);
...
uses mdiform,setaxes, MEPCFreq, Printgra, Printrec ;
...
begin
     ...
     cbDCChannel.ItemIndex := MinInt([1,CdrFH.NumChannels-1]) ;
     ...
     { Create cursors for AC and DC channel displays }
     DataCursors.DCZero := scDisplay.AddHorizontalCursor(DCChan,clGray,True) ;
     if cbDCChannel.ItemIndex <> cbACChannel.ItemIndex then
        DataCursors.ACZero := scDisplay.AddHorizontalCursor(ACChan,clGray,True) ;
     ...
     VarCursors.Fit0 := plVarPlot.AddVerticalCursor( clGray ) ;
     VarCursors.Fit1 := plVarPlot.AddVerticalCursor( clGray ) ;
     VarCursors.Read := plVarPlot.AddVerticalCursor( clRed ) ;
     lbVarFit0.Visible := False ;
     lbVarFit1.Visible := False ;
     shVarFitLine.Visible := False ;
     lbVarRead.Visible := False ;

     { Initialise power spectrum plot display cursors }
     plSpecPlot.ClearVerticalCursors ;
     SpecCursors.Fit0 := plSpecPlot.AddVerticalCursor( clGray ) ;
     SpecCursors.Fit1 := plSpecPlot.AddVerticalCursor( clGray ) ;
     SpecCursors.Read := plSpecPlot.AddVerticalCursor( clRed ) ;
     lbSpecFit0.Visible := False ;
     lbSpecFit1.Visible := False ;
     lbSpecRead.Visible := False ;
...
procedure TPwrSpecFrm.InitialiseDisplay(
          scDisplay : TScopeDisplay   { Drawing area }
          ) ;
{ ------------------------------------------------
...
begin
     ...
     scDisplay.TFormat := ' %.6g ' + Settings.TUnits + ' ' ;
...
begin
    ...
    scDisplay.HorizontalCursors[DataCursors.DCZero] := Channel[DCChan].ADCZero ;
    if DCChan <> ACCHan then
       scDisplay.HorizontalCursors[DataCursors.ACZero] := Channel[ACChan].ADCZero ;

    scDisplay.xOffset := Round(Channel[DCChan].xMin) ;
    scDisplay.SetDataBuf( ADC^ ) ;

    end ;


procedure TPwrSpecFrm.DisplayMeanAndVariance ;
{ ----------------------------------------------
...
procedure TPwrSpecFrm.ReadRecord(
          Rec : Integer ;
          var Buf : Array of SmallInt ;
          var xMin,xMax : single
          ) ;
...
procedure TPwrSpecFrm.SetRecordSize(
          NewSize : Integer ;
          SizeChange : Boolean 
          ) ;
{ -------------------------------
...
begin
     ...
     NumRecords := CdrFH.NumSamplesInFile div
     ...
     if Data.MaxRecord > High(TRecordStatusArray) then begin
        MessageDlg( format( ' Record storage capacity exceeded (%d)',
...
{    Change record type / rejected array if record size/overlap changed }
     ...
     if SizeChange then begin
        try
           ...
           for Rec := 0 to MaxInt([Data.MaxRecord,OldMaxRecord]) do begin
...
begin
     ...
     prVarProgress.Max := High(Variance.EndAt) ;
     prVarProgress.Min := 1 ;
     prVarProgress.Position := 1 ;
     prVarProgress.Max := Data.MaxRecord ;
...
begin
     try
        ...
        if VarFunc.Equation = Exponential then
           xOffset := MinFlt([plVarPlot.VerticalCursors[VarCursors.Fit0],
...
begin
     ...
     if MEPCFreqFrm.ModalResult = mrOK then begin
        ...
        erVarResults.Lines.Add( format(' Avg. Variance = %.4g %s^2',
                             [AvgVariance,Channel[ACChan].ADCUnits] ) ) ;

        erVarResults.Lines.Add( format(' Avg. Skew = %.4g %s^3',
                             [AvgSkew,Channel[ACChan].ADCUnits] ) ) ;

        erVarResults.Lines.Add( format(' MEPC Frequency = %f /s',
                                        [MEPCFrequency] ) ) ;

        erVarResults.Lines.Add( format(' No. records = %d',[nAvg]) ) ;

        end ;

     end;

procedure TPwrSpecFrm.plVarPlotCursorChange(Sender: TObject);
{ -----------------------------------------------
  Update labels when variance plot cursors change
  -----------------------------------------------}
var
   x,y : single ;
begin
     ...
     plVarPlot.GetPoint( VarDataLine,
                         plVarPlot.FindNearestIndex(VarDataLine,VarCursors.Read),
                          x,y ) ;
     lbVarRead.caption := format('%.4g %s / %4.g %s ',
                           [x,GetVariableUnits(cbVarXAxis.ItemIndex),
                            y,GetVariableUnits(cbVarYAxis.ItemIndex)] ) ;

     lbVarRead.Visible := True ;
     lbVarRead.Left := plVarPlot.Left
                        + plVarPlot.XToCanvasCoord( x ) - (lbVarRead.Width div 2);

     { Set Fitting/area cursor labels }
     lbVarFit0.Visible := True ;
     lbVarFit0.Left := plVarPlot.Left +
                       plVarPlot.XToCanvasCoord(
                       plVarPlot.VerticalCursors[VarCursors.Fit0] ) ;
     lbVarFit1.Visible := True ;
     lbVarFit1.Left := plVarPlot.Left +
                       plVarPlot.XToCanvasCoord(
                       plVarPlot.VerticalCursors[VarCursors.Fit1] ) ;

     { Place horizontal line between baseline block cursors }
     shVarFitLine.visible := true ;
     shVarFitLine.Top := lbVarFit0.Top + (lbVarFit0.Height div 2) ;
     shVarFitLine.Left := MinInt([lbVarFit0.Left,lbVarFit1.Left])
                         + lbVarFit0.Width ;
     shVarFitLine.Width := MaxInt([lbVarFit0.Left,lbVarFit1.Left])
                          - shVarFitLine.Left - lbVarFit1.Width ;
...
begin
    ...
    plSpecPlot.CreateLine( SpecBackLine, clBlue, msOpenTriangle, psClear ) ;
    if ckSpecSubtractBackground.checked then begin
       for i := 0 to PowerSpectrum^.NumPoints-1 do begin
...
begin
     ...
     try
        ...
        { Initialise progress bar }
        prSpecProgress.Min := 1 ;
        prSpecProgress.Max := High(EndAt) ;
        prSpecProgress.Position := StartAt ;        
        prSpecProgress.Max := EndAt + 1 ;
        prSpecProgress.Min := StartAt ;
        ...
        for Rec := StartAt to EndAt do begin
            if RecordStatus^[Rec].Valid
               ...
               if RecType = Test then edSpecStatus.text := 'Test'
                                 else edSpecStatus.text := 'Back' ;
               prSpecProgress.Position := Rec ;
     ...
     { Initialise progress bar }
     prSpecProgress.Position := prSpecProgress.Min ;
     ...
     Try
        ...
        if OK then begin
           ...
           for i := 0 to SpecFunc.NumParameters-1 do
               if not SpecFunc.FixedParameters[i] then begin
                  SpecFunc.Parameters[i] := SpecFunc.InitialGuess(
                                            FitData^, nFit, i ) ;
                  end ;
        ...
        if OK then begin
           ...
           case SpecFunc.Equation of
                ...
                { MEPC noise spectrum }
                MEPCNoise : begin
                   Tau := SecsToMs / (2.0*Pi*SpecFunc.Parameters[2]) ;
                   SpecResults.Add( format(' ^st^-R = %.4g ms',[Tau]) ) ;
                   Tau := SecsToMs / (2.0*Pi*SpecFunc.Parameters[1]) ;
                   SpecResults.Add( format(' ^st^-D = %.4g ms',[Tau]) ) ;
                   end ;
...
begin
     ...
     scDisplay.Width := page.Width - scDisplay.Left - 20 ;
     plVarPlot.Width := page.Width - plVarPlot.Left - 20 ;
     plSpecPlot.Width := page.Width - plSpecPlot.Left - 20 ;

     Bottom := page.ClientHeight - 40 ;
     ControlGrp.Height := Bottom - ControlGrp.Top ;
     DataGrp.Top := Bottom - DataGrp.Height ;
     DataGrp.Left := scDisplay.Left ;
     DataGrp.Width := scDisplay.Width ;

     scDisplay.Height := DataGrp.Top - scDisplay.Top - 10  ;

     { Extend variance page control group to bottom of page }
     VarGrp.Height := page.Height - VarGrp.Top - 40 ;
     prVarProgress.Top := VarGrp.Height - prVarProgress.Height - 4 ;
     ...
     plVarPlot.Height := VarFitGrp.Top - plVarPlot.Top
                            - 2*lbVarFit0.Height - 2 ;
     { Cursor labels }
     lbVarFit0.Top := plVarPlot.Top + plVarPlot.Height + 2;
     lbVarFit1.Top := lbVarFit0.Top ;
     lbVarRead.Top := lbVarFit1.Top + lbVarFit1.Height + 2 ;

     { Extend spectrum page control group to bottom of page }
     SpecGrp.Height := page.Height - SpecGrp.Top - 40 ;
     edSpecStatus.Top := SpecGrp.Height - edSpecStatus.Height - 2 ;
     prSpecProgress.Top := edSpecStatus.Top ;
     ...
     plSpecPlot.Height := SpecFitGrp.Top - plSpecPlot.Top
                            - lbSpecRead.Height
                            - lbSpecFit0.Height
                            - lbArea.Height ;
...
procedure TPwrSpecFrm.cbDCChannelChange(Sender: TObject);
{ --------------------------------------------
...
begin
     ...
     { Recreate cursors for AC and DC channel displays }
...
procedure TPwrSpecFrm.bSetRecordStateClick(Sender: TObject);
{ -----------------------------------
...
procedure TPwrSpecFrm.cbACChannelChange(Sender: TObject);
{ --------------------------------------------
...
begin
     ...
     // Recreate horizontal cursors
     scDisplay.ClearHorizontalCursors ;
     // DC cursor
     DataCursors.DCZero := scDisplay.AddHorizontalCursor(DCChan,clGray,True) ;
     scDisplay.HorizontalCursors[DCChan] := Channel[DCChan].ADCZero ;
     // AC cursor
     if DCChan <> ACChan then begin
        DataCursors.ACZero := scDisplay.AddHorizontalCursor(ACChan,clGray,True) ;
        scDisplay.HorizontalCursors[ACChan] := Channel[ACChan].ADCZero ;
        end ;

     scDisplay.xOffset := Round(Channel[ACChan].xMin) ;
     scDisplay.SetDataBuf( ADC^ ) ;
     end;
...
procedure TPwrSpecFrm.plSpecPlotCursorChange(Sender: TObject);
{ -----------------------------------------------
  Update labels when spectrum plot cursors change
  -----------------------------------------------}
var
   x,y,Freq0,Freq1,FreqLo,FreqHi,Variance,y0,y1 : single ;
   i : Integer ;
begin

     { Set readout cursor label }
     plSpecPlot.GetPoint( SpecDataLine,
                          plSpecPlot.FindNearestIndex(SpecDataLine,SpecCursors.Read),
                          x,
                          y ) ;
     lbSpecRead.caption := format(' %.4g Hz / %.4g %s ',
                           [x,y,Channel[ACChan].ADCUnits+'^2'] ) ;
     lbSpecRead.Visible := True ;
     lbSpecRead.Top := plSpecPlot.Top + plSpecPlot.Height ;
     lbSpecRead.Left := plSpecPlot.Left
                        + plSpecPlot.XToCanvasCoord( x )
                        - (lbSpecRead.Width div 2);


     { Set Fitting/area cursor labels }
     lbSpecFit0.Visible := True ;
     lbSpecFit0.Top := lbSpecRead.Top + lbSpecRead.Height - 4 ;
     ...
     shLine.Left :=  MinInt([lbSpecFit0.Left + lbSpecFit0.Width,lbSpecFit1.Left]) + 2 ;
     shLine.Width := MaxInt([lbSpecFit0.Left + lbSpecFit0.Width,lbSpecFit1.Left])
...
procedure TPwrSpecFrm.SetCopyAndPrintMenus ;
{ --------------------------------
...
{ -----------------------------------------------------
  ...
  ----------------------------------------------------- }
begin
      if Page.ActivePage = DataTab then scDisplay.ZoomIn(cbACChannel.ItemIndex);
     end ;
...
procedure TPwrSpecFrm.RecordStatusFile(
...
begin
     ...
     lbBaseline.Left := scDisplay.Left + scDisplay.Width + 1 ;
     lbBaseline.Top := scDisplay.Top - (lbBaseline.Height div 2)
                       + scDisplay.YToScreenCoord(DCChan,
                         scDisplay.HorizontalCursors[DataCursors.DCZero]) ;
